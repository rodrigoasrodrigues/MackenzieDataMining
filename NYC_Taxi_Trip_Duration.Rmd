...
Optamos por utilizar apenas os dados fornecidos pelo Kaggle para análise, não incluimos em nosso estudo fontes de dados adicionais fornecidas nos foruns de discussão.
...
Para a resolução do problema utilizamos 2 modelos diferentes apresentados aqui por ordem de efetividade.

## KMeans

Antes de aplicar os modelos, utilizamos o Kmeans como ferramenta de análise exploratória, para entender melhor a distribuição dos dados.
...
Com esse volume de dados obtivemos a acurácia de 0.59

![Resultado no Kaggle.com](prints/nnet13k.png)

Utilizando 10 % dos dados obtivemos a acurácia de 0.55

![Resultado no Kaggle.com](prints/nnet130k.png)

# Conclusão

Concluímos através do desenvolvimento do presente trabalho, que é possível utilizar diferentes tecnologias para desenvolver um modelo preditivo, dentre as quais, utilizamos o Kmeans para classificação da gravação da viagem, Random Forest e Neural Networks para responder o desafio proposto no Kaggle e verificar a melhor estimativa para a duraçao da viagem de taxi.
Acreditamos que há margem para melhorias pois as bses utilizadas necessitam de limpeza adicional, visto que a que por nós foi executa gerou um resultado de erro médio quadrado que pode ser influenciado por outros tipos de parâmetros e montagem de outros modelos que levariam a uma melhor acertividade.

# Crux

As dificuldades enfrentadas se deram em relação ao tamanho das bases de treino e teste originais, a realização da limpeza dos dados para tentar melhorar a análise, quais bibliotecas importar e saber quais os melhores modelos para solucionar o problema proposto.
Sabemos que não existe um modelo padrão ou bala de prata, sendo a análise preditiva e a mineração de dados uma questão de arte, tecnologia e estratégia.
